...
void commit(Repository* repo, const char* message, UndoStack* undo_stack) {
    ...
    // Store the filenames without copying them immediately
    for (int i = 0; i < repo->staged_count; i++) {
        new_commit->files[i] = repo->staging_area[i];
    }

    repo->staged_count = 0; // Clear staging area after commit
...
void push_origin_master(Repository* repo) {
    ...
    while (current) {
        ...
        for (int i = 0; i < current->file_count; i++) {
            ...
            sprintf(commit_filename, "%d_%s", current->id, current->files[i].filename);
            sprintf(destination, "%s/%s", commit_dir, commit_filename);
            FILE* src_file = fopen(current->files[i].filename, "r");
            ...
            fclose(src_file);
            fclose(dest_file);
            printf("File '%s' committed as '%s'.\n", current->files[i].filename, destination);
        }
